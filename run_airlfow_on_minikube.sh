...
helm upgrade --install airflow apache-airflow/airflow --set dags.persistence.enabled=true --set dags.persistence.existingClaim=host-dags-pvc --set dags.gitSync.enabled=false --set executor=KubernetesExecutor --set logs.persistence.enabled=true --set images.airflow.repository=felixneko/fs_etl_basic --set images.airflow.tag=latest